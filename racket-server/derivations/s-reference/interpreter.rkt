...
(define current-closure-observer (make-parameter (lambda (_family _procedure _captures) (void))))
...
(define (prefix owners search)
  (match search
    ...
    [`(Yield ,local ,answer ,rest) `(Yield ,(owners-append owners local) ,answer ,rest)]
...
(define (eval/s goal state owners inherited)
  ...
  (match (goal-body goal)
    [`(∃ ,binders ,body ,tag)
     (let ([intro (fresh-names (owners-support owners inherited) (length binders))])
...
(define (bind/s search continue owners inherited)
  (match search
    ...
    [`(Yield ,local (Answer ,answer ,state) ,rest)
	 (let* ([common (owners-append owners local)]
			[here (owners-support common inherited)])
...
(define (force/s search inherited)
  (match-let ([`(Delay ,owners ,resume) search])
	(resume owners inherited)))
...
(define (commit/s search)
  (match search
    ...
    [`(Yield ,owners ,answer ,rest) `(Emit ,owners ,answer ,(commit/s rest))]
...
(define (advance/s frontier inherited)
  (match frontier
    [`(Done ,_)                      frontier]
    [`(Solo ,_ ,_)                   frontier]
    [`(Emit ,owners ,answer ,rest)   `(Emit ,owners ,answer ,(advance/s rest (owners-support owners inherited)))]
    [`(Forced ,owners ,rest)         `(Forced ,owners ,(advance/s rest (owners-support owners inherited)))]
    [`(More (Delay ,owners ,resume)) `(Forced ,owners ,(commit/s (resume '(Owners) (owners-support owners inherited))))]))

(define (collect/s frontier inherited)
  (match frontier
    [`(Done ,_)                      frontier]
    [`(Solo ,_ ,_)                   frontier]
    [`(Emit ,owners ,answer ,rest)   `(Emit ,owners ,answer ,(collect/s rest (owners-support owners inherited)))]
    [`(Forced ,owners ,rest)         `(Forced ,owners ,(collect/s rest (owners-support owners inherited)))]
    [`(More (Delay ,owners ,resume)) (let ([here (owners-support owners inherited)])
