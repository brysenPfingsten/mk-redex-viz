#lang racket
;; Generated from defunc.rkt by register-derive.rkt. Regenerate; do not edit.
(require (only-in
          "../shared/kernel.rkt"
          owners-support
          owners-append
...
(define (dispatch! bank)
  (match
   ...
   ('outcome/d
    (define outcome (Registers-r0 bank))
    (define failure (Registers-r1 bank))
    (define success (Registers-r2 bank))
    (begin
      (match
       outcome
       ((Failure) (jump! bank 'failure/d failure))
       ((Success state) (jump! bank 'success/d success state)))))
   ('failure/d
    (define failure (Registers-r0 bank))
    (begin (match failure ((FEmpty owners k) (jump! bank 'return/d `(Empty ,owners) k)))))
   ('success/d
    (define success (Registers-r0 bank))
    (define state (Registers-r1 bank))
    (begin (match success ((SOne owners k) (jump! bank 'return/d `(One ,owners ,state) k)))))
   ('return/d
