...
(define call-lazy-extra/l3
  (reduction-relation
    ...
    [--> (Γ (in-hole Kdisj (in-hole Kconj (proceed ((r t ... tag) σ)))) as)
         (Γ (in-hole Kdisj (in-hole Kconj (g_new σ))) as)
         (where g_new ,(instantiate-call-host (term Γ) (term r) (term (t ...))))
         "l3-base/lazy-expand-on-resume"]

    [--> (Γ (in-hole Kdisj (in-hole Kconj ((delay s_1) × g c))) as)
         (Γ (in-hole Kdisj (in-hole Kconj (delay (s_1 × g c)))) as)
         "l3-base/delay-through-conj"]))

(define Rl3-base-lazy
...
(define (step-once prog)
  (step-once/deterministic Rl3-base-lazy prog))
